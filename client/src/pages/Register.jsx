...
export default function Register({ setUser }) {
  ...
  const submit = async (e) => {
    ...
    setError('')
    try {
      const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/api/auth/register`, { email, password })
  ...
  return (
    <form onSubmit={submit} className="max-w-md mx-auto mt-10 p-6 bg-gray-800 rounded">
      ...
      <button type="submit" className="w-full bg-indigo-600 py-2 rounded hover:bg-indigo-700 transition">Register</button>
    </form>
  )
}
